...
class EnrolledCampaigns extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      token:null,
  ...
  async getCampaigns() {
    try {
      await apiClient.post('campaign/enrolled', 
      { },
  ...
  async disenrollCampaign(campaign) {
    try {
      await apiClient.delete('user/enroll', 
      { 
        headers: {Authorization: 'Bearer ' + this.state.token},
        data: {post_id: campaign}
      }).then(r => {
        console.log(r.data);
      }).catch(e => { 
        console.log(e);
  ...
  renderCard = (item, index) => {
    ...
    return (
        <Card style={{width: cardWidth}} item={item} key={item._id} cta publications subitem={item.campaign} screenavigation={'Post'} full />
    );
  };
  ...
  render() {
    ...
    const showConfirmDialog = (rowMap, rowKey) => {
      return Alert.alert(
        "Are your sure?",
        "Are you sure you want to remove this enrollement?",
        [
          {
            text: "Yes",
            onPress: () => {
    ...
    const deleteRow = (rowMap, rowKey) => {
        ...
        this.disenrollCampaign(campaign._id);
